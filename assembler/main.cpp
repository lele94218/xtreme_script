...
void InitInstrTable() {
    ...
    // The following code makes repeated calls to AddInstrLookup () to add a
    // hardcoded
    // instruction set to the assembler's vocabulary. Each AddInstrLookup () call
    // is
    // followed by zero or more calls to SetOpType (), whcih set the supported
    // types of
    ...
    SetOpType(iInstrIndex, 1,
              OP_FLAG_TYPE_INT | OP_FLAG_TYPE_FLOAT | OP_FLAG_TYPE_STRING |
    ...
    SetOpType(iInstrIndex, 0, OP_FLAG_TYPE_MEM_REF | OP_FLAG_TYPE_REG);
    SetOpType(iInstrIndex, 1,
              OP_FLAG_TYPE_MEM_REF | OP_FLAG_TYPE_REG | OP_FLAG_TYPE_STRING);
    ...
    SetOpType(iInstrIndex, 1,
              OP_FLAG_TYPE_MEM_REF | OP_FLAG_TYPE_REG | OP_FLAG_TYPE_STRING);
    SetOpType(iInstrIndex, 2,
              OP_FLAG_TYPE_MEM_REF | OP_FLAG_TYPE_REG | OP_FLAG_TYPE_INT);
...
int main(int argc, char *argv[]) {
    ...
    // may
    // include appending file extensions if they aren't present, and possibly
    // copying the
