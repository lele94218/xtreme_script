...
static struct string
{
    ...
    int len;
    struct string *link;
} * buckets[1024]; // the length of buckets should be power of 2
    ...
    for (p = buckets[h]; p; p = p->link)
    {
        if (p->len == len)
        {
            ...
            do
            {
                if (s1 == end)
                {
                    /* string exist in the hash list, return the address of this string. */
                    return p->str;
                }
            } while (*s1++ == *s2++);
        }
    }
    /* string is not in hash list */
